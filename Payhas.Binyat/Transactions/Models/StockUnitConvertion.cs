// Decompiled with JetBrains decompiler
// Type: Payhas.Binyat.Transactions.Models.StockUnitConvertion
// Assembly: Payhas.Binyat, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 19F85A6C-D40F-439D-9478-41F01000D67D
// Assembly location: C:\Users\Admin\AppData\Local\Temp\Bofyhol\f9d7aa10a6\lib\net45\Payhas.Binyat.dll

using Payhas.Data.Models;
using System;

#nullable disable
namespace Payhas.Binyat.Transactions.Models;

public class StockUnitConvertion : BindableObject
{
  private string _id;
  private string _stockId;
  private string _unitId;
  private Decimal _multiplier;
  private Decimal _divider;

  public StockUnitConvertion() => this.Id = Guid.NewGuid().ToString();

  public string Id
  {
    get => this._id;
    set => this.SetProperty<string>(ref this._id, value, nameof (Id));
  }

  public string StockId
  {
    get => this._stockId;
    set => this.SetProperty<string>(ref this._stockId, value, nameof (StockId));
  }

  public string UnitId
  {
    get => this._unitId;
    set => this.SetProperty<string>(ref this._unitId, value, nameof (UnitId));
  }

  public Decimal Multiplier
  {
    get => this._multiplier;
    set => this.SetProperty<Decimal>(ref this._multiplier, value, nameof (Multiplier));
  }

  public Decimal Divider
  {
    get => this._divider;
    set => this.SetProperty<Decimal>(ref this._divider, value, nameof (Divider));
  }
}

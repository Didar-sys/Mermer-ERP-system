...
// Type: Payhas.Binyat.Core.Couch.Common.CommonCouchViewsCreator
...
public class CommonCouchViewsCreator : IInitialSchemaCreator
{
  private readonly ICouchCluster _cluster;

  public CommonCouchViewsCreator(ICouchCluster cluster) => this._cluster = cluster;

  public async Task CreateAsync(bool includeReporting)
  {
    CommonCouchViewsCreator couchViewsCreator = this;
    string designDoc = await couchViewsCreator.GetType().Assembly.ReadResourceAsync("common.json");
    using (IBucket bucket = couchViewsCreator._cluster.OpenDefaultBucket())
    {
      using (IBucketManager manager = bucket.CreateManager())
      {
        IResult result = await manager.UpdateDesignDocumentAsync("common", designDoc);
        if (!result.Success)

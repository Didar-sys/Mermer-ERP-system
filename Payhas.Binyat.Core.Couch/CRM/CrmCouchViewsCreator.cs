...
// Type: Payhas.Binyat.Core.Couch.CRM.CrmCouchViewsCreator
...
#nullable disable
namespace Payhas.Binyat.Core.Couch.CRM;

public class CrmCouchViewsCreator : IInitialSchemaCreator
{
  private readonly ICouchCluster _cluster;

  public CrmCouchViewsCreator(ICouchCluster cluster) => this._cluster = cluster;

  public async Task CreateAsync(bool includeReporting)
  {
    CrmCouchViewsCreator couchViewsCreator = this;
    string designDoc = await couchViewsCreator.GetType().Assembly.ReadResourceAsync("crm.json");
    using (IBucket bucket = couchViewsCreator._cluster.OpenDefaultBucket())
    {
      using (IBucketManager manager = bucket.CreateManager())
      {
        IResult result = await manager.UpdateDesignDocumentAsync("crm", designDoc);
        if (!result.Success)

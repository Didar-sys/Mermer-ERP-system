...
// Type: Payhas.Binyat.Core.Couch.Activations.Services.BinyatActivationService
...
#nullable disable
namespace Payhas.Binyat.Core.Couch.Activations.Services;
...
public class BinyatActivationService : IBinyatActivationService
{
  ...
  private async Task<IEnumerable<ActivationResult>> GetActivationResultsAsync(string machineId)
  {
    ...
    {
      using (IBucket bucket = this._cluster.OpenDefaultBucket())
        return (IEnumerable<ActivationResult>) (await bucket.GetDocumentAsync<ActivationResultDocument>(this.GetId(machineId))).Content.ActivationResults;
    }
    catch
    {
      return (IEnumerable<ActivationResult>) Array.Empty<ActivationResult>();
    }
  }
